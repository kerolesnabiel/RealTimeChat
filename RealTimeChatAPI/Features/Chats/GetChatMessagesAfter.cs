...
internal static class GetChatMessagesAfter
{
    public sealed record ChatMessagesResponse(IReadOnlyList<ChatMessageDto> Messages, bool HasMoreAfter);
        ...
        IUserContext userContext) : IQueryHandler<Query, ChatMessagesResponse>
    {
        ...
        public async Task<ChatMessagesResponse> Handle(Query query, CancellationToken cancellationToken)
        {
            ...
            var hasMoreAfter = messages.Count > MessagesCount;
            if (hasMoreAfter)
                messages.RemoveAt(messages.Count - 1);

            return new ChatMessagesResponse(messages, hasMoreAfter);
        }
    }
    ...
    public sealed class Endpoint : IEndpoint
    {
        ...
        {
            app.MapGet("api/chats/{chatId}/messages/after/{messageId}", async (
                Guid chatId,
                Guid messageId,
